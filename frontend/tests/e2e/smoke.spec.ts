...
test.describe("Smoke Tests", () => {
  ...
  test("insights page loads successfully", async ({ page }) => {
    ...
    const datePickerButtons = page
      .locator('button:has([data-testid="calendar-icon"]), button:has(svg)')
      .filter({ hasText: /Pick a date|Start Date|End Date/ })
  ...
  test("language switcher is present", async ({ page }) => {
    ...
    await expect(
      page.locator('button[data-testid="language-switcher"]')
    ).toBeVisible()
  })
})

...
test.describe("Language Switching", () => {
  test("can switch between German and English", async ({ page }) => {
    ...
    const languageSwitcherText = page.locator(
    ...
    // On mobile, navigation links may be hidden, so check the language switcher itself
    const languageSwitcherTextDE = page.locator(
      'button[data-testid="language-switcher"] span'
    )
  ...
  test("language preference persists after page reload", async ({ page }) => {
    ...
    // On mobile, navigation links may be hidden, so check the language switcher itself
    const languageSwitcherTextEN = page.locator(
      'button[data-testid="language-switcher"] span'
    )
    ...
    const languageSwitcherAfterReload = page.locator(
      'button[data-testid="language-switcher"] span'
    )
    ...
    if (switcherText && switcherText.includes("en")) {
      ...
      console.log(
        "Language preference did not persist after reload - reverted to German"
      )
    }
  })
  ...
  test("language switching works on insights page", async ({ page }) => {
    ...
    await expect(page.locator("h1")).toBeVisible()
    await expect(
      page.locator('label:has-text("Station")').first()
    ).toBeVisible()

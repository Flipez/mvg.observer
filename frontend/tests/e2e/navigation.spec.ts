...
test.describe("Navigation and Tabs", () => {
  ...
  test("header navigation links work", async ({ page }) => {
    ...
    const insightsLink = (await insightsLinkDE.isVisible())
      ? insightsLinkDE
      : insightsLinkEN
    ...
    const overviewLink = (await overviewLinkDE.isVisible())
      ? overviewLinkDE
      : overviewLinkEN
  ...
  test("mobile navigation works", async ({ page }) => {
    ...
    if (await mobileMenuButton.isVisible()) {
      ...
      await expect(
        page.locator('a:has-text("Insights"), a:has-text("Einblicke")')
      ).toBeVisible()
    }

    // Tab navigation should still work on mobile
    const tableTab = page.locator(
      'button:has-text("Table"), button:has-text("Tabelle")'
    )

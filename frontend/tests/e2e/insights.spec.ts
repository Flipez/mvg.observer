...
test.describe('Insights Page', () => {
  ...
  test('station dropdown search functionality works', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Marienplatz")').first()).toBeVisible();
    ...
    await expect(page.locator('button[role="combobox"]:has-text("Marienplatz")')).toBeVisible();
  });
  ...
  test('station dropdown filters results correctly', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Hauptbahnhof")').first()).toBeVisible();
  ...
  test('date pickers are functional', async ({ page }) => {
    ...
    const datePickerButtons = page.locator('button:has(svg)').filter({ hasText: /\d/ }); // Look for buttons with dates
  ...
  test('station selection shows analytics cards', async ({ page }) => {
    ...
    if (await gridContainer.isVisible()) {
      ...
      if (await anyCards.count() > 0) {
        ...
        await expect(page.locator('button[role="combobox"]')).toContainText('Marienplatz');
      }
    }
  });
  ...
  test('excluded stations are not in dropdown', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Frankfurt Hbf")')).not.toBeVisible();
    await expect(page.locator('button:has-text("Nürnberg Hbf")')).not.toBeVisible();
